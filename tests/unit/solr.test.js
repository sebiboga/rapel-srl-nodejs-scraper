...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { id: 'job1', url: 'https://test.com/1', cif: '5665609' },
        { id: 'job2', url: 'https://test.com/2', cif: '5665609' }
      ]));
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('5665609')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('5665609')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '5665609', company: 'RAPEL SRL', brand: 'RAPEL' }
      ]));

      const result = await solr.queryCompanySOLR('id:33159615');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('RAPEL');
      expect(result.docs[0].company).toBe('RAPEL SRL');
    });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('5665609')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('5665609')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/job1', title: 'Job 1', cif: '5665609' },
        { url: 'https://test.com/job2', title: 'Job 2', cif: '5665609' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '5665609' },
    ...
    it('should detect invalid CIF format', async () => {
      ...
      for (const job of result.docs) {
        expect(job.cif).not.toMatch(/^\d{2,10}$/);
      }
    });
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '5665609', status: 'scraped' },
        { url: 'https://test.com/2', title: 'Job 2', cif: '5665609', status: 'verified' },
        { url: 'https://test.com/3', title: 'Job 3', cif: '5665609', status: 'published' }
      ]));
