...
This scraper extracts job listings from jobRapid.ro (RAPEL SRL jobs) and imports them to peviitor.ro.

Target: https://www.jobrapid.ro
...
8. **Scrape new jobs** - Extract jobs from jobRapid.ro via HTML scraping with cheerio
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts RAPEL jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from jobRapid.ro via HTML scraping
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape jobRapid.ro (via cheerio HTML scraping)
    │
    ▼
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real scraping (jobRapid.ro) + ANAF + SOLR |
